...
import { Chekcking } from "../models/chekcking";
...
export class Checks {
  ...
  registerChekcking(data) {
    console.log('llego aqui');
    return new Promise(resolve => {
      let headers = new Headers();
      headers.append('Content-Type', 'application/json');
      this.http.post(this.server + '/api/checks/create', JSON.stringify(data), {headers: headers})
        .map(res => res.json())
        .subscribe(data => {
          console.log(data);
          resolve(data);
  ...
  getAllChecks() {
    return new Promise(resolve => {
      let headers = new Headers();
      headers.append('Content-Type', 'application/json');
      this.http.get(this.server + '/api/checks/read', {headers: headers})
        .map(res => res.json())
        .subscribe(data => {
          resolve(data);
        });
